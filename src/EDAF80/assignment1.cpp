...
main()
{
    ...
    SpinConfiguration const sun_spin{
            glm::radians(0.0f),
            glm::two_pi<float>() / 6.0f};

    glm::vec3 const mercury_scale{0.02f};
    SpinConfiguration const mercury_spin{
            glm::radians(-0.0f),
            glm::two_pi<float>() / 180.0f};
    OrbitConfiguration const mercury_orbit{
            2.0f,
            glm::radians(-3.4f),
            glm::two_pi<float>() / 4.0f};

    glm::vec3 const venus_scale{0.05f};
    SpinConfiguration const venus_spin{
            glm::radians(-2.6f),
            -glm::two_pi<float>() / 600.0f};
    OrbitConfiguration const venus_orbit{
            3.0f,
            glm::radians(-3.9f),
            glm::two_pi<float>() / 12.0f};

    glm::vec3 const earth_scale{0.05f};
    SpinConfiguration const earth_spin{
            glm::radians(-23.0f),
            glm::two_pi<float>() / 3.0f};
    OrbitConfiguration const earth_orbit{
            4.0f,
            glm::radians(-7.2f),
            glm::two_pi<float>() / 20.0f};

    glm::vec3 const moon_scale{0.01f};
    SpinConfiguration const moon_spin{
            glm::radians(-6.7f),
            glm::two_pi<float>() / 90.0f};
    OrbitConfiguration const moon_orbit{
            0.2f,
            glm::radians(29.0f),
            glm::two_pi<float>() / 1.3f};

    glm::vec3 const mars_scale{0.03f};
    SpinConfiguration const mars_spin{
            glm::radians(-25.0f),
            glm::two_pi<float>() / 3.0f};
    OrbitConfiguration const mars_orbit{
            5.0f,
            glm::radians(-5.7f),
            glm::two_pi<float>() / 36.0f};

    glm::vec3 const jupiter_scale{0.5f};
    SpinConfiguration const jupiter_spin{
            glm::radians(-3.1f),
            glm::two_pi<float>() / 1.0f};
    OrbitConfiguration const jupiter_orbit{
            13.0f,
            glm::radians(-6.1f),
            glm::two_pi<float>() / 220.0f};

    glm::vec3 const saturn_scale{0.4f};
    SpinConfiguration const saturn_spin{
            glm::radians(-27.0f),
            glm::two_pi<float>() / 1.2f};
    OrbitConfiguration const saturn_orbit{
            16.0f,
            glm::radians(-5.5f),
    ...
    SpinConfiguration const uranus_spin{
            glm::radians(-82.0f),
            -glm::two_pi<float>() / 2.0f};
    OrbitConfiguration const uranus_orbit{
            18.0f,
            glm::radians(-6.5f),
            glm::two_pi<float>() / 1680.0f};

    glm::vec3 const neptune_scale{0.2f};
    SpinConfiguration const neptune_spin{
            glm::radians(-28.0f),
            glm::two_pi<float>() / 2.0f};
    OrbitConfiguration const neptune_orbit{
            19.0f,
            glm::radians(-6.4f),
    ...
    GLuint const mercury_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_mercury.jpg"));
    ...
    GLuint const moon_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_moon.jpg"));
    GLuint const mars_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_mars.jpg"));
    GLuint const jupiter_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_jupiter.jpg"));
    GLuint const saturn_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_saturn.jpg"));
    GLuint const saturn_ring_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_saturn_ring_alpha.png"));
    GLuint const uranus_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_uranus.jpg"));
    GLuint const neptune_texture = bonobo::loadTexture2D(
            config::resources_path("planets/2k_neptune.jpg"));

    //
    // Set up the celestial bodies.
    //

    auto sun = CelestialBody{sphere, &celestial_body_shader, sun_texture};
    sun.set_scale(sun_scale);
    sun.set_spin(sun_spin);

    auto mercury =
            CelestialBody{sphere, &celestial_body_shader, mercury_texture};
    mercury.set_scale(mercury_scale);
    mercury.set_spin(mercury_spin);
    mercury.set_orbit(mercury_orbit);
    sun.add_child(&mercury);

    auto venus = CelestialBody{sphere, &celestial_body_shader, venus_texture};
    venus.set_scale(venus_scale);
    venus.set_spin(venus_spin);
    venus.set_orbit(venus_orbit);
    sun.add_child(&venus);

    auto earth = CelestialBody{sphere, &celestial_body_shader, earth_texture};
    earth.set_scale(earth_scale);
    earth.set_spin(earth_spin);
    earth.set_orbit(earth_orbit);

    auto moon = CelestialBody{sphere, &celestial_body_shader, moon_texture};
    ...
    auto mars = CelestialBody{sphere, &celestial_body_shader, mars_texture};
    ...
    auto jupiter =
            CelestialBody{sphere, &celestial_body_shader, jupiter_texture};
    jupiter.set_scale(jupiter_scale);
    jupiter.set_spin(jupiter_spin);
    jupiter.set_orbit(jupiter_orbit);
    sun.add_child(&jupiter);

    auto saturn = CelestialBody{sphere, &celestial_body_shader, saturn_texture};
    ...
    auto uranus = CelestialBody{sphere, &celestial_body_shader, uranus_texture};
    ...
    auto neptune =
            CelestialBody{sphere, &celestial_body_shader, neptune_texture};
    ...
    // Render stack, add root, root adds children and so on
    struct CelestialBodyRef {
        CelestialBody* body;
        glm::mat4 parent_transform;
    };

    auto renderStack    = std::stack<CelestialBodyRef>();
    auto const rootNode = CelestialBodyRef{&sun, glm::mat4{1.0f}};
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        renderStack.push(rootNode);

        while(!renderStack.empty()) {
            auto const [parent, transform] = renderStack.top();
            renderStack.pop();

            auto const child_transform = parent->render(
            ...
            for(auto&& child : parent->get_children()) {
                renderStack.push({child, child_transform});
            }
        }
