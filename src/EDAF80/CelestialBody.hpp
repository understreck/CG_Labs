...
struct SpinConfiguration {
    float axial_tilt{0.0f};    //!< Angle in radians between the body's
                               //!< rotational and orbital axis.
    float speed{0.0f};         //!< Rotation speed in radians per second.
};

struct OrbitConfiguration {
    float radius{0.0f};    //!< Distance in metres between the centre of the
                           //!< orbit and the body's centre of gravity.
    float inclination{0.0f};    //!< Angle in radians between the body's orbital
                                //!< axis and its parent's rotational axis.
    float speed{0.0f};          //!< Rotation speed in radians per second.
};
...
public:
    ...
    //! \brief Mark another celestial body as being “attached” to the current
    //! one.
    void
    add_child(CelestialBody* child);
    ...
    void
    set_orbit(OrbitConfiguration const& configuration);

    //! \brief Configure the scale of this celestial body.
    void
    set_scale(glm::vec3 const& scale);

    //! \brief Configure the spin parameters for this celestial body.
    void
    set_spin(SpinConfiguration const& configuration);
...
private:
    struct {
        Node node;
        struct {
            float radius{0.0f};    //!< Distance in metres between its centre of
                                   //!< gravity and the centre of the orbit.
            float inclination{0.0f};    //!< Angle in radians between the its
                                        //!< orbital axis and its parent's
                                        //!< rotational axis.
            float speed{0.0f};    //!< Rotation speed in radians per second.
            float rotation_angle{0.0f};    //!< How much has it rotated around
                                           //!< its orbital axis; in radians.
        } orbit;
        glm::vec3 scale{1.0f};
        struct {
            float axial_tilt{0.0f};    //!< Angle in radians between the its
                                       //!< rotational and orbital axis.
            float speed{0.0f};    //!< Rotation speed in radians per second.
            float rotation_angle{0.0f};    //!< How much has it rotated around
                                           //!< its rotational axis; in radians.
        } spin;
    } _body;
