...
bonobo::init()
{
    ...
    local::fullscreen_shader = bonobo::createProgram(
...
bonobo::loadObjects(std::string const& filename)
{
    ...
    auto const parent_folder  = (end_of_basedir != std::string::npos
                                         ? filename.substr(0, end_of_basedir)
                                         : ".")
                               + "/";
    ...
    if(assimp_scene->mNumMeshes == 0u) {
        LogError(
                "No mesh available; loading \"%s\" must have had issues",
                filename.c_str());
        return objects;
    }
    ...
    for(size_t j = 0; j < assimp_scene->mNumMeshes; ++j) {
        ...
        if(material_id >= assimp_scene->mNumMaterials)
            LogError(
                    "Mesh \"%s\" has a material index of %u, but only %u materials are present.",
    ...
    for(size_t i = 0; i < assimp_scene->mNumMaterials; ++i) {
        ...
        auto const material_start_time =
        ...
        auto const process_texture = [&bindings,
                                      &material,
                                      i,
                                      &parent_folder,
                                      &texture_count](
            ...
            if(material->GetTextureCount(type)) {
                ...
                if(material->GetTextureCount(type) > 1)
                    LogWarning(
                            "Material \"%s\" has more than one %s texture: discarding all but the first one.",
                            material->GetName().C_Str(),
                            type_as_str.c_str());
                aiString path;
                material->GetTexture(type, 0, &path);
                auto const id = bonobo::loadTexture2D(
                        parent_folder + std::string(path.C_Str()));
                if(id == 0u) {
                    LogWarning(
                            "Failed to load the %s texture for material \"%s\".",
                            type_as_str.c_str(),
                            material->GetName().C_Str());
                    return;
                }
                ...
                auto const texture_end_time =
                        std::chrono::high_resolution_clock::now();
                LogTrivia(
                        "│ %s Texture \"%s\" loaded in %.3f ms",
                        bindings.size() == 1 ? "┌" : "├",
                        path.C_Str(),
                        std::chrono::duration<float, std::milli>(
                                texture_end_time - texture_start_time)
                                .count());
            }
        };
        ...
        auto const material_end_time =
                std::chrono::high_resolution_clock::now();
        LogTrivia(
                "│ %s Material \"%s\" loaded in %.3f ms",
                bindings.empty() ? "╺" : "┕",
                material->GetName().C_Str(),
                std::chrono::duration<float, std::milli>(
                        material_end_time - material_start_time)
                        .count());
    }
    ...
    for(size_t j = 0; j < assimp_scene->mNumMeshes; ++j) {
        ...
        if(!assimp_object_mesh->HasFaces()) {
            LogError(
                    "Unsupported mesh \"%s\": has no faces",
                    assimp_object_mesh->mName.C_Str());
            continue;
        }
        if((assimp_object_mesh->mPrimitiveTypes
            & ~static_cast<uint32_t>(
                    aiPrimitiveType_POINT | aiPrimitiveType_NGONEncodingFlag))
                   != 0u
           && (assimp_object_mesh->mPrimitiveTypes
               & ~static_cast<uint32_t>(
                       aiPrimitiveType_LINE | aiPrimitiveType_NGONEncodingFlag))
                      != 0u
           && (assimp_object_mesh->mPrimitiveTypes
               & ~static_cast<uint32_t>(
                      ...
                      != 0u) {
            LogError(
                    "Unsupported mesh \"%s\": uses multiple primitive types",
                    assimp_object_mesh->mName.C_Str());
            continue;
        }
        if((assimp_object_mesh->mPrimitiveTypes
           ...
           == static_cast<uint32_t>(aiPrimitiveType_POLYGON)) {
            LogError(
                    "Unsupported mesh \"%s\": uses polygons",
                    assimp_object_mesh->mName.C_Str());
            continue;
        }
        if(!assimp_object_mesh->HasPositions()) {
            LogError(
                    "Unsupported mesh \"%s\": has no positions",
                    assimp_object_mesh->mName.C_Str());
            continue;
        }
        ...
        auto const tangents_size =
                assimp_object_mesh->HasTangentsAndBitangents() ? vertices_size
                                                               : 0u;
        ...
        if(assimp_object_mesh->HasNormals()) {
            ...
            glEnableVertexAttribArray(static_cast<unsigned int>(
        ...
        if(assimp_object_mesh->HasTextureCoords(0u)) {
            glBufferSubData(
                    ...
                    texcoords_offset,
                    texcoords_size,
                    static_cast<GLvoid const*>(
                            assimp_object_mesh->mTextureCoords[0u]));
            glEnableVertexAttribArray(static_cast<unsigned int>(
        ...
        if(assimp_object_mesh->HasTangentsAndBitangents()) {
            ...
            glBufferSubData(
                    ...
                    binormals_size,
                    static_cast<GLvoid const*>(
                            assimp_object_mesh->mBitangents));
            glEnableVertexAttribArray(static_cast<unsigned int>(
        ...
        object.indices_nb =
                assimp_object_mesh->mNumFaces * num_vertices_per_face;
        auto object_indices = std::make_unique<GLuint[]>(
                static_cast<size_t>(object.indices_nb));
        for(size_t i = 0u; i < assimp_object_mesh->mNumFaces; ++i) {
            ...
            if(num_vertices_per_face >= 1u)
                object_indices[num_vertices_per_face * i + 1u] =
                        face.mIndices[1u];
            if(num_vertices_per_face >= 2u)
                object_indices[num_vertices_per_face * i + 2u] =
                        face.mIndices[2u];
        }
        ...
        glBufferData(
                ...
                static_cast<unsigned int>(object.indices_nb)
                        * sizeof(GL_UNSIGNED_INT),
                reinterpret_cast<GLvoid const*>(object_indices.get()),
                GL_STATIC_DRAW);
        ...
        std::string attributes =
                assimp_object_mesh->HasNormals() ? "normals" : "";
        if(!attributes.empty())
            attributes += " | ";
        if(assimp_object_mesh->HasTangentsAndBitangents())
        ...
        LogTrivia(
                ...
                (assimp_scene->mNumMeshes == 1u)
                        ? "╶"
                        : (j == 0 ? "┌"
                                  : (j == assimp_scene->mNumMeshes - 1 ? "└"
                                                                       : "├")),
                assimp_object_mesh->mName.C_Str(),
                attributes.c_str(),
                std::chrono::duration<float, std::milli>(
                        mesh_end_time - mesh_start_time)
                        .count());
    }
    ...
    LogInfo("┕ Scene loaded in %.3f s: %u textures loaded in %.3f s and %zu meshes in %.3f s",
            std::chrono::duration<float>(scene_end_time - scene_start_time)
                    .count(),
            texture_count,
            std::chrono::duration<float>(
                    materials_end_time - materials_start_time)
                    .count(),
            objects.size(),
...
[[nodiscard]] auto
upload_CubeMap_face(
        std::string const& facePath,
        GLenum const faceEnum,
        GLuint texture) -> bool
{
    ...
    // texels.
    std::uint32_t width, height;
    auto data = getTextureData(facePath, width, height, false);
    if(data.empty()) {
        LogWarning(
                ("Failed to get texture data from path: " + facePath).c_str());
        glDeleteTextures(1, &texture);
        return false;
    }
    ...
    glTexImage2D(
            ...
            /* the height of the cube map's face */
            static_cast<GLsizei>(height),
            /* must always be 0 */ 0,
        ...
        bool generate_mipmap)
{
    ...
    if(!upload_CubeMap_face(posx, GL_TEXTURE_CUBE_MAP_POSITIVE_X, texture)) {
        std::cerr << "Failed to upload " + posx << "\n";
        return 0u;
    }

    if(!upload_CubeMap_face(posy, GL_TEXTURE_CUBE_MAP_POSITIVE_Y, texture)) {
        std::cerr << "Failed to upload " + posy << "\n";
        return 0u;
    }

    if(!upload_CubeMap_face(posz, GL_TEXTURE_CUBE_MAP_POSITIVE_Z, texture)) {
        std::cerr << "Failed to upload " + posz << "\n";
        return 0u;
    }

    if(!upload_CubeMap_face(negx, GL_TEXTURE_CUBE_MAP_NEGATIVE_X, texture)) {
        std::cerr << "Failed to upload " + negx << "\n";
        return 0u;
    }

    if(!upload_CubeMap_face(negy, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, texture)) {
        std::cerr << "Failed to upload " + negy << "\n";
        return 0u;
    }

    if(!upload_CubeMap_face(negz, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, texture)) {
        std::cerr << "Failed to upload " + negz << "\n";
        return 0u;
    }
...
bonobo::createProgram(
        std::string const& vert_shader_source_path,
        std::string const& frag_shader_source_path)
{
    ...
    GLuint program = utils::opengl::shader::generate_program(
        ...
        float farPlane)
{
    ...
    auto const viewport_origin = glm::ivec2(
            relative_to_absolute(lower_left.x, window_size.x),
            relative_to_absolute(lower_left.y, window_size.y));
    auto const viewport_size =
            glm::ivec2(
                    relative_to_absolute(upper_right.x, window_size.x),
    ...
    glUniform4iv(
            glGetUniformLocation(local::fullscreen_shader, "swizzle"),
            1,
            glm::value_ptr(swizzle));
    glUniform1i(
            glGetUniformLocation(local::fullscreen_shader, "linearise"),
            linearise);
    glUniform1f(
            glGetUniformLocation(local::fullscreen_shader, "near"),
            nearPlane);
    glUniform1f(
            glGetUniformLocation(local::fullscreen_shader, "far"),
            farPlane);
...
GLuint
bonobo::createFBO(
        std::vector<GLuint> const& color_attachments,
        GLuint depth_attachment)
{
    ...
    for(size_t i = 0; i < color_attachments.size(); ++i)
        attach(static_cast<GLenum>(GL_COLOR_ATTACHMENT0 + i),
               color_attachments[i]);
        ...
        glm::mat4 const& world)
{
    ...
    glUniformMatrix4fv(
            basis.shader_locations.world,
            1,
            GL_FALSE,
            glm::value_ptr(world));
    glUniformMatrix4fv(
            basis.shader_locations.view_proj,
            1,
            GL_FALSE,
...
setupBasisData()
{
    ...
    assert(basis.ibo != 0);
    std::array<glm::u8vec3, 16> const indices = {// Body: Left
                                                 glm::u8vec3(0u, 1u, 2u),
                                                 glm::u8vec3(0u, 2u, 3u),
                                                 // Body: Back
                                                 glm::u8vec3(4u, 0u, 3u),
    ...
    shader_location =
            glGetUniformLocation(basis.shader, "vertex_world_to_clip");
    assert(shader_location >= 0);
