...
void main() {
	...
	frag_color = vec4(
		ambient_colour +
		diffuse_colour * diffuse_texture_colour * max(dot(fs_in.normal, fragPosToLight), 0.0) +
		specular_colour * specular_texture_colour *
			pow(max(dot(reflect(-fragPosToLight, fs_in.normal), fragPosToCamera), 0.0), shininess_value)
		,
		1.0f);
}
